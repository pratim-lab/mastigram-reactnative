import { View, Dimensions, TouchableOpacity, FlatList, StatusBar, Platform, Alert } from 'react-native'
...
const TestRecords = ({ navigation }) => {
  ...
  const emailFormik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      testIds: Yup.mixed().test({
        ...
        test: val => checkedData.length === 1
      })
    }),
    onSubmit: (values, actions) => {
      sendReporMultiAction(values, actions, setOpenCheckbox, setOpenEmail, setEmailAddress)
    }
  })
  ...
  const rightSwipeActions = (row) => {
    return (
      <TouchableOpacity
        onPress={() => {
          deleteMultiAction({ testIds: [row?.id] }, setOpenCheckbox)
        }}
        style={{
      ...
      <View style={{
        ...
        {index === 0 &&
          <View style={{
            ...
            {openCheckbox ?
              <TouchableOpacity
                onPress={() => setOpenCheckbox(false)}>
                <Text style={{ fontFamily: GothamBook }}>Done</Text>
              </TouchableOpacity> :
              <TouchableOpacity
                onPress={() => setOpenCheckbox(true)}>
